...
1. [Kiến trúc tổng thể](#1-kiến-trúc-tổng-thể)
   ...
   - [1.3 Kênh giao tiếp giữa các thành phần](#13-kênh-giao-tiếp-giữa-các-thành-phần)
   - [1.4 Phân tích ảnh hưởng khi sự cố](#14-phân-tích-ảnh-hưởng-khi-một-thành-phần-gặp-sự-cố)
2. [C4 Diagram](#2-c4-diagram)
   - [Level 1 — System Context](#level-1--system-context)
   - [Level 2 — Container](#level-2--container)
3. [High-Level Architecture Diagram](#3-high-level-architecture-diagram)
   - [3.1 Luồng tích hợp Legacy SIS](#31-luồng-tích-hợp-legacy-sis--csv-import)
   - [3.2 Luồng tích hợp Payment Gateway](#32-luồng-tích-hợp-payment-gateway-với-circuit-breaker)
   - [3.3 Luồng tích hợp AI Model](#33-luồng-tích-hợp-ai-model)
   - [3.4 Luồng check-in offline](#34-luồng-check-in-offline-và-đồng-bộ)
4. [Thiết kế Dữ liệu Hệ thống (PostgreSQL & Redis)](#4-thiết-kế-dữ-liệu-hệ-thống-postgresql--redis)
   - [4.1 Phân tích các loại dữ liệu chính](#41-phân-tích-các-loại-dữ-liệu-chính-trong-hệ-thống)
   - [4.2 Đề xuất loại Database phù hợp](#42-đề-xuất-loại-database-phù-hợp)
   - [4.3 Thiết kế Schema trên PostgreSQL](#43-thiết-kế-schema-trên-postgresql-dữ-liệu-cố-định)
   - [4.4 Thiết kế Cấu trúc Dữ liệu trên Redis](#44-thiết-kế-cấu-trúc-dữ-liệu-trên-redis-in-memory-store)
...
| ------------------------ | ------------------------------------------------------------ | --------------------- |
| **Student Web App**      | Giao diện sinh viên: xem workshop, đăng ký, nhận QR          | ReactJS               |
| **Admin Web App**        | Giao diện ban tổ chức: quản lý workshop, thống kê            | ReactJS               |
| **Mobile App**           | Nhân sự quét QR, offline-first                               | React Native + SQLite |
| **Backend API**          | Xử lý nghiệp vụ, auth, bảo vệ hệ thống                       | NestJS                |
...
```mermaid
    flowchart TD
        ...
        subgraph unihub_boundary ["Hệ thống UniHub Workshop"]
            ...
            web_student["**Student Web App**<br/>[ReactJS]<br/><br/>Xem lịch, đăng ký workshop,<br/>hiển thị QR & thông báo"]:::container
            web_admin["**Admin Web App**<br/>[ReactJS]<br/><br/>Quản lý workshop, upload PDF,<br/>thống kê đăng ký"]:::container
            mobile_app["**Mobile App**<br/>[React Native + SQLite]<br/><br/>Quét QR check-in,<br/>lưu offline vào SQLite local"]:::container

            backend_api["**Backend API**<br/>[NestJS]<br/><br/>Toàn bộ nghiệp vụ: auth, RBAC,<br/>đăng ký, check-in, rate limit,<br/>circuit breaker, idempotency"]:::container
            ...
            broker{{"**RabbitMQ**<br/>[RabbitMQ 3.x]<br/><br/>Message broker:<br/>notify.*, ai.pdf.*"}}:::container
        ...
        backend_api -->|"Publish events<br/>notify, ai<br/>[AMQP]"| broker
...
```mermaid
    sequenceDiagram
        ...
        alt Key đã tồn tại (duplicate request)
            ...
            API->>RD: SETNX lock:workshop:{workshop_id} (10s TTL)
            alt Không lấy được lock (slot đang tranh chấp)
                API-->>SV: 409 Conflict — thử lại sau
            else Lock thành công
                API->>RD: DECR slots:workshop:{workshop_id}
                RD-->>API: slots = N

                alt slots < 0
                    API->>RD: INCR slots:workshop:{workshop_id}
                    API->>RD: DEL lock:workshop:{workshop_id}
                    API-->>SV: 400 — Hết chỗ
                else slots >= 0
                    ...
                    alt CB = OPEN
                        ...
                        alt Thanh toán thành công
                            ...
                            API->>PG: INSERT registration; INSERT payment (DB transaction)
...
_(Ràng buộc UNIQUE kết hợp `student_id` và `workshop_id`)_
